...
- **Mode Nespresso** (bouton en haut de l'app) : bascule TOUTE l'app dans un calcul simplifié — ventes au prix TTC tel que payé par le client, moins achats matières tels qu'entrés, sans rien retirer d'autre (ni TVA, ni charges fixes, ni amortissement, ni cotisations, ni provision impôt). Les mêmes ventes déjà saisies sont réutilisées. Un bouton doré indique quand ce mode est actif ; une icône "?" à côté explique la différence avec le mode complet. **Ce chiffre est toujours plus élevé que le vrai bénéfice net — à ne pas confondre lors de la lecture des comptes.** ⚠️ **Sécurité** : une confirmation explicite avec avertissement est désormais obligatoire avant toute bascule (activation ou désactivation), pour éviter qu'un appui accidentel fausse la lecture du bénéfice sans s'en rendre compte.

## 7. Nouvel onglet "Cumuls" — bénéfices agrégés, projections, analyse produits
